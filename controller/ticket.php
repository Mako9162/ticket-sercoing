<?php
    ...
    switch ($_GET["op"]) {
        case "insert":
            $datos = $ticket->insert_ticket($_POST["usu_id"], $_POST["cat_id"], $_POST["cats_id"], $_POST["cos_id"], $_POST["tick_titulo"], $_POST["tick_descrip"]);
        ...
        case "listar":
            ...
            foreach ($datos as $row) {
                ...
                if ($row["usu_asig"]==null) {
                    ...
                    $datos1=$usuario->get_usuario_x_id($row["usu_asig"]);
                    foreach ($datos1 as $row1) {
                        $sub_array[] = '<a onClick="reasignar('.$row["tick_id"].');"><span class="label label-pill label-success">'.$row1["usu_nom"].' '.$row1["usu_ape"].'</span></a>';
                    }
                }
        ...
        case "mostrar":
            ...
            if(is_array($datos)==true and count($datos)>0){
                foreach($datos as $row){
                    ...
                    $output["tick_est_texto"] = $row["tick_estado"];
        ...
        // case "grafico1":
        //     $datos=$ticket->get_ticket_grafico1();
        //     echo json_encode($datos);   
        // break;
